...
#pragma config(Motor,  port2,           motorGoliath,  tmotorServoContinuousRotation, openLoop)
...
#pragma config(Motor,  port6,           motorDriveRightMiddleBack, tmotorServoContinuousRotation, openLoop, reversed) //Expander C and D INVERTED WIRE
#pragma config(Motor,  port5,           motorDriveLeftMiddleBack, tmotorServoContinuousRotation, openLoop) // Expander A and B
#pragma config(Motor,  port7,           motorDriveRightFront, tmotorServoContinuousRotation, openLoop, reversed)
#pragma config(Motor,  port8,           motorMoGoLift, tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port9,           motorMini4BarRight, tmotorServoContinuousRotation, openLoop, reversed)
#pragma config(Motor,  port10,          motorArmRight, tmotorServoContinuousRotation, openLoop, reversed)
...
#define ARM_POTENTIOMETER_MAX_VALUE					2600
#define ARM_POTENTIOMETER_STATIONARY_GOAL_VALUE		2120 // UNTESTED
#define ARM_POTENTIOMETER_CONE_HEIGHT_VALUE  		1530 // UNTESTED
#define ARM_POTENTIOMETER_CONE_STACK_INITIAL_VALUE	1530 // UNTESTED
#define ARM_POTENTIOMETER_CONE_MULTIPLIER			60 // UNTESTED
...
#define MINI_4_BAR_POTENTIOMETER_EXTENDED_VALUE 	2718
#define MINI_4_BAR_POTENTIOMETER_RETRACTED_VALUE 	1100
#define MINI_4_BAR_POTENTIOMETER_PARALLEL_VALUE		2800
#define MINI_4_BAR_POTENTIOMETER_OFFSET				0
#define MINI_4_BAR_POTENTIOMETER_MULTIPLIER 		1
#define MINI_4_BAR_POWER_EXTEND						80
...
#define MENU_LIST_AUTON_LENGTH 			13
...
MenuItem menuItemAutonGoBack, menuItemAutonD;
//side menu items
...
void populateMenuItems() {
	...
	menuItemAutonD.name = "AutonD";
	menuItemAutonD.id = ++id;
	menuItemAutonD.idx = ++i;
	menuItemAutonD.isDirectional = true;
	menuListAuton[i] = &menuItemAutonD;
...
byte getDriveLeftMotorPower() { return motor[motorDriveLeftFront]; }
byte getDriveRightMotorPower() { return motor[motorDriveRightFront]; }
byte getArmMotorPower() { return motor[motorArmLeft]; }
byte getMini4BarMotorPower() { return motor[motorMini4BarLeft]; }
...
void setDriveMotorPower(int leftMotorPower, int rightMotorPower) // Set Drive motor power with seperate power for each side
{
	...
	motor[motorDriveRightMiddleBack] = rightMotorPower * rightDriveMultiplier;

	motor[motorDriveLeftFront] = leftMotorPower * leftDriveMultiplier;
	motor[motorDriveLeftMiddleBack] = leftMotorPower * leftDriveMultiplier;
}
...
{
	motor[motorMini4BarLeft] = motorPower;
	motor[motorMini4BarRight] = motorPower;
}

void setMoGoLiftMotorPower(int motorPower)
{
	motor[motorMoGoLift] = motorPower;
}
...
void drivePIDControl(short goalPoint, Mode mode, OnStall onStall, bool isDataLogged)
{
	...
	if (mode == MODE_ACCURATE || mode == MODE_MOGO)
	{
		...
		if (mode == MODE_ACCURATE)
		{
			while (abs(error) > 250 && ( (time1[T4] - timeInitialOnStall < 1000 && onStall == ON_STALL_EXIT) || onStall == ON_STALL_NOTHING) )
			{
				errorDifference = error - (goalPoint - getDriveLeftSensorValue());
				error = goalPoint - getDriveLeftSensorValue();

				if (isDataLogged) DataLogPID(error, 0, errorDifference, 127 * error / abs(error), 0, 0, 127 * error / abs(error));
		...
		else if (mode == MODE_MOGO)
		{
			while (abs(error) > 250 && ( (time1[T4] - timeInitialOnStall < 1000 && onStall == ON_STALL_EXIT) || onStall == ON_STALL_NOTHING) )
			{
				...
				if (isDataLogged) DataLogPID(error, 0, errorDifference, SlewRate(getDriveLeftMotorPower(), 127 * 0.67 * error / abs(error), 0.05, 5), 0, 0, SlewRate(getDriveLeftMotorPower(), 127 * 0.67 * error / abs(error), 0.05, 5));
		...
		while (time1[T4] - timeInitialPID < 150 && ( (time1[T4] - timeInitialOnStall < 1000 && onStall == ON_STALL_EXIT) || onStall == ON_STALL_NOTHING) )
		{
			...
			else if (error < 0) newPower = -15 + error * pGain + errorSum * iGain - errorDifference * dGain;
...
void mini4BarPIDControl(short goalPoint, WaitForAction stopWhenMet, OnStall onStall, bool isDataLogged)
{
	...
	while ( (time1[T4] - timeInitialPID < 150 || stopWhenMet == WAIT_NONE) && ( (time1[T4] - timeInitialOnStall < 1000 && onStall == ON_STALL_EXIT) || onStall == ON_STALL_NOTHING) )
	{
		...
		if (abs(error) > 90) timeInitialPID = time1[T4];
		if ( !(abs(errorDifference) < 5 && abs(newPower) >= 127.0) ) timeInitialOnStall = time1[T4];

		//if (abs(error) <= 90 && goalPoint > correctMini4BarGoalPoint(MINI_4_BAR_POTENTIOMETER_RETRACTED_VALUE + 200) ) errorSum = -25.0 / iGain;
...
void mini4BarRetract(WaitForAction stopWhenMet, OnStall onStall)
{
	...
	while (getMini4BarSensorValue() > correctMini4BarGoalPoint(MINI_4_BAR_POTENTIOMETER_RETRACTED_VALUE)  && ( (time1[T4] - timeInitialOnStall < 1000 && onStall == ON_STALL_EXIT) || onStall == ON_STALL_NOTHING) )
	{
...
void moGoRetract(OnStall onStall)
{
	...
	while (getMoGoLiftSensorValue() > correctMoGoLiftGoalPoint(MOGO_LIFT_POTENTIOMETER_RETRACTED_VALUE)  && ( (time1[T4] - timeInitialOnStall < 1000 && onStall == ON_STALL_EXIT) || onStall == ON_STALL_NOTHING) )
	{
...
void moGoExtend(OnStall onStall)
{
	...
	while (getMoGoLiftSensorValue() < correctMoGoLiftGoalPoint(MOGO_LIFT_POTENTIOMETER_EXTENDED_VALUE) && ( (time1[T4] - timeInitialOnStall < 2000 && onStall == ON_STALL_EXIT) || onStall == ON_STALL_NOTHING) )
	{
...
task autonomous()
{
	...
	if ( (*selectedProgram).id == menuItemAutonD.id)
	{
